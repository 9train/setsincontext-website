...
2. A main step title.
3. A compact step status label such as `Step 1 of 2` or `Step 1 of 3`.
4. A subtle progress row such as `Privacy • Ready` or `Join method • Details • Ready`.
...
- Middle steps: Back on the left, Next on the right
- Final step: Back remains available, but the primary action belongs to the step itself
...
The website assumes sessions run through the deployed remote runtime. Start Session
must not ask the host to choose a runtime mode.
...
- if remote runtime environment variables are missing, host/viewer actions must stay disabled and honest

## Start Session Acceptance Criteria

1. The modal is a strict 2-step wizard.
2. Step 1 asks only Private vs Public.
3. Step 2 is a clean ready state.
4. Step 2 includes session title, host name, and room key.
5. Room key regeneration lives on the final step.
6. Back navigation preserves state.
7. Host links use the deployed remote runtime.
8. Viewer links use the deployed remote runtime.
9. The old dense summary/dashboard feel is absent.
10. Private session behavior remains fully honest.
11. No runtime-engine scope creep is introduced.
