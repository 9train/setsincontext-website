...
import { buildResolvedViewerSessionLink, buildSessionResolveUrl, buildHostSessionLink, buildViewerSessionLink, parseViewerJoinInput, createSessionRuntimeTargets, resolveViewerSessionByKey, resolveSessionRuntimeTarget, sanitizeRoomKey, } from "../src/sessionLinks.js";
...
test("local host links use the localhost runtime and preserve metadata", () => {
    ...
    const link = new URL(buildHostSessionLink({
        room: "Practice Room",
        mode: "local",
        visibility: "private",
...
test("generated session links default to the remote runtime", () => {
    ...
    const hostLink = new URL(buildHostSessionLink({
        room: "Shared Room",
        visibility: "private",
        hostAccessToken: "host-token-123",
    }, env));
    const viewerLink = new URL(buildViewerSessionLink({
        room: "Shared Room",
        visibility: "public",
    }, env));
    ...
    assert.equal(`${viewerLink.origin}${viewerLink.pathname}`, "https://runtime.ddj.test/viewer.html");
    assert.equal(viewerLink.searchParams.get("mode"), "remote");
    assert.equal(`${resolveUrl.origin}${resolveUrl.pathname}`, "https://runtime.ddj.test/api/sessions/resolve");
...
test("session resolve URLs reuse the centralized runtime target and keep private access when present", () => {
    const resolveUrl = new URL(buildSessionResolveUrl("  Practice Room  ", "local", undefined, {
        accessToken: "invite-token-1",
    }));
    assert.equal(`${resolveUrl.origin}${resolveUrl.pathname}`, "http://localhost:8080/api/sessions/resolve");
...
test("remote viewer links stay honest with placeholder targets until configured", () => {
    ...
    const link = new URL(buildViewerSessionLink({
        room: "Public Lesson",
        mode: "remote",
        visibility: "public",
        viewerName: "Ari",
    ...
    assert.equal(`${link.origin}${link.pathname}`, "https://runtime.example.invalid/viewer.html");
...
test("resolved viewer links trust same-origin runtime joinUrlPath and fall back on unsafe origins", () => {
    ...
    }, runtimeTarget, { accessToken: "invite-token-1" });
    assert.equal(trustedLink, "http://localhost:8080/viewer.html?room=practice-room&ws=ws%3A%2F%2Flocalhost%3A8787%2F");
    assert.equal(fallbackLink, "http://localhost:8080/viewer.html?room=practice-room&ws=ws%3A%2F%2Flocalhost%3A8787&access=invite-token-1");
});
test("parseViewerJoinInput extracts room and access from a pasted private invite link", () => {
    const parsed = parseViewerJoinInput("http://localhost:8080/viewer.html?room=practice-room&ws=ws%3A%2F%2Flocalhost%3A8787&access=invite-token-1");
...
test("resolveViewerSessionByKey returns a safe viewer URL for known runtime sessions", async () => {
    ...
    assert.equal(requestedUrl, "http://localhost:8080/api/sessions/resolve?key=practice-room&access=invite-token-1");
    ...
    assert.equal(result.viewerUrl, "http://localhost:8080/viewer.html?room=practice-room&ws=ws%3A%2F%2Flocalhost%3A8787%2F&access=invite-token-1");
});
...
test("resolveViewerSessionByKey maps private invite enforcement failures to clean UI-safe errors", async () => {
    ...
    assert.equal(inviteRequired.ok ? "" : inviteRequired.code, "private-invite-required");
    assert.equal(invalidInvite.ok, false);
    assert.equal(invalidInvite.ok ? "" : invalidInvite.code, "invalid-private-invite");
});
